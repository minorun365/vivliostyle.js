...
describe("cross-reference layout constraint", function () {
  ...
  it("re-resolves references while a target keeps moving earlier", function () {
    const store = new Counters.CounterStore(documentURLTransformer);
    const reference = new Counters.TargetCounterReference("target", true);
    store.pageIndicesById.target = { spineIndex: 0, pageIndex: 2 };
    store.resolvedReferences.target = [reference];
    ...
    expect(earlierConstraint.allowLayoutAfterPageBreak(nodeContext)).toBe(true);
    ...
    store.pageIndicesById.target = { spineIndex: 0, pageIndex: 2 };
    const repeatedConstraint = store.createLayoutConstraint(1);
    expect(repeatedConstraint.allowLayoutAfterPageBreak(nodeContext)).toBe(
      false,
    );
  });
});
